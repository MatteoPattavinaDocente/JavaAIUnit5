...
export function useNotifiche(utente: string) {
  ...
  const [pagina, setPagina] = useState<Pagina<NotificationDto>>(PAGINA_VUOTA)
   ...
   *  fra "a tutti", "a chi segue l'ordine 42" e "a te". */
  const [pubbliche, setPubbliche] = useState<BustaRicevuta[]>([])

  /** Gli ordini che questa sessione ha chiesto di seguire. */
  const [ordiniSeguiti, setOrdiniSeguiti] = useState<number[]>([])
  ...
  const seguitiRef = useRef<number[]>(ordiniSeguiti)
  ...
  useEffect(() => {
    ...
    const apri = () => {
      ...
      socket.onmessage = (evento) => {
        ...
        let busta: Busta
        try {
          busta = JSON.parse(evento.data as string) as Busta
        } catch {
   ...
   */
  const segui = useCallback((ordine: number) => {
   ...
   */
  const pubblica = useCallback((ordine: number, testo: string) => {
    const socket = socketRef.current
    if (socket?.readyState !== WebSocket.OPEN) {
  ...
  const smetti = useCallback((ordine: number) => {
    const socket = socketRef.current
    if (socket?.readyState === WebSocket.OPEN) {
